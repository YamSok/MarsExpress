...
class PdData(RawData):
    ...
    def get_df_dict(self, year):
        ...
        for k in d.keys():
            d[k] = pd.read_csv(d[k])
            # d[k].index = pd.to_datetime(d[k].index, unit='ms') # ut_ms -> datetime
            # d[k] = d[k].resample('60min').pad()
            # print(d[k])
    ...
    def add_dmop_data(self, master, d, no_power_sub = False):
        ...
        dummies = pd.get_dummies(d['dmop']['subsystem'].apply(lambda x: x[:4])).ewm(span=100).mean()
